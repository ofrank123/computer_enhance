...
char *getFlagsStr(u16 flags) {
    char *ret = allocStr(10);
    u8 offset = 0;
    if (flags & FLAG_CF) ret[offset++] = 'C';
    if (flags & FLAG_PF) ret[offset++] = 'P';
    if (flags & FLAG_AF) ret[offset++] = 'A';
    if (flags & FLAG_ZF) ret[offset++] = 'Z';
    if (flags & FLAG_SF) ret[offset++] = 'S';
    if (flags & FLAG_TF) ret[offset++] = 'T';
    if (flags & FLAG_IF) ret[offset++] = 'I';
    if (flags & FLAG_DF) ret[offset++] = 'D';
    if (flags & FLAG_OF) ret[offset++] = 'O';
    ret[offset] = 0;
    return ret;
}
...
void executeInstr(Instr instr) {
    u16 preOpFlags = regFile.flags;
    u16 preOpIP = regFile.ip;

    regFile.ip += instr.length;
    ...
    switch (instr.op) {
        ...
        case OP_JNE:
            if (!(regFile.flags & FLAG_ZF))
                regFile.ip += readArg(instr.dst);
            break;
        case OP_JE:
            if (regFile.flags & FLAG_ZF)
                regFile.ip += readArg(instr.dst);
            break;
        case OP_JP:
            if (regFile.flags & FLAG_PF)
                regFile.ip += readArg(instr.dst);
            break;
        case OP_JB:
            if (regFile.flags & FLAG_CF) 
                regFile.ip += readArg(instr.dst);
            break;
        case OP_LOOPNZ:
            writeReg(REG_CX, readReg(REG_CX) - 1);
            if (readReg(REG_CX) != 0 && !(regFile.flags & FLAG_ZF)) 
                regFile.ip += readArg(instr.dst);
            break;
        default:
    ...
    printf(" ip:0x%X->0x%X", preOpIP, regFile.ip);

    if (regFile.flags != preOpFlags) {
        printf(" flags %s->%s", getFlagsStr(preOpFlags), getFlagsStr(regFile.flags));
    }
}
...
void executeProgram() {
    ...
    InstrFlags flags;
    while (nextByte != 0x0f) {
        decodeNextInstr(&instr, regFile.ip);
        handleFlags(&flags, &instr);
        printf("%s ; ", getInstrStr(instr));
        executeInstr(instr);
        printf("\n");
        readMem(&nextByte, regFile.ip, 1);
    }
    ...
    printf("\tip: 0x%04X (%d)\n", regFile.ip, regFile.ip);

    printf("flags: ");
    printf("%s", getFlagsStr(regFile.flags));
    printf("\n");
}
